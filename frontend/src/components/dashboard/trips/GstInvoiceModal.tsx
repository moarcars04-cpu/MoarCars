...
export const GstInvoiceModal: React.FC<GstInvoiceModalProps> = ({ booking, isOpen, onClose }) => {
  ...
  const rawBookingId = booking.bookingId || `MC-2026-${booking.id || "8812"}`;
  ...
  let invoiceDate = new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
  if (booking.createdAt) {
    try {
      const d = new Date(booking.createdAt);
      if (!isNaN(d.getTime())) {
        invoiceDate = d.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
      }
    } catch {}
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-black/85 backdrop-blur-md animate-in fade-in overflow-y-auto">
      ...
      <div className="relative w-full max-w-4xl max-h-[96vh] rounded-2xl sm:rounded-3xl bg-white text-slate-900 shadow-2xl flex flex-col overflow-hidden border border-slate-200">
        ...
        <div className="sticky top-0 z-20 px-4 sm:px-6 py-3 bg-slate-900 text-white flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 print:hidden">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider truncate">
              Original GST Tax Invoice • Rule 46 CGST (1-Page A4)
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              onClick={handlePrint}
              className="px-3 sm:px-4 py-1.5 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-slate-950 text-[11px] sm:text-xs font-black flex items-center gap-1.5 transition-all shadow-sm"
            >
        ...
        <div
          ...
          className="printable-document p-4 sm:p-6 md:p-8 overflow-y-auto space-y-3.5 text-slate-900 text-[11px] leading-tight bg-white"
        >
          
          {/* 1. Official Header & Company Info */}
          <div className="invoice-block flex flex-col sm:flex-row justify-between items-start gap-3 border-b-2 border-slate-900 pb-3">
            <div className="space-y-0.5">
              <div className="flex items-center gap-2">
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-950 text-amber-400 font-black text-xs shrink-0">
                  M
                </span>
              ...
              <p className="text-[10.5px] text-slate-600 font-medium">
                Station Road, Opp. Tirupati Central Railway Station Hub, Tirupati, AP - 517501
              </p>
              <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[10px] font-mono text-slate-700 pt-0.5">
              ...
              <p className="text-[9.5px] text-slate-500">
                CIN: U50100AP2026PTC012345 • Helpline: +91 96664 99904 • www.moarcars.com
              </p>
            </div>

            <div className="sm:text-right space-y-0.5 shrink-0 w-full sm:w-auto">
              <span className="inline-block px-2.5 py-0.5 rounded bg-slate-950 text-amber-400 text-[10px] font-black uppercase tracking-wider">
                TAX INVOICE
              </span>
              <p className="text-[9px] uppercase tracking-wider text-slate-500 font-bold block">
          ...
          <div className="invoice-block grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200">
            {/* Customer Details */}
            <div className="space-y-0.5">
              <span className="text-[9px] uppercase tracking-wider font-black text-slate-500 block">
                Details of Receiver / Billed To:
              </span>
              <p className="text-xs font-black text-slate-950">{booking.customerName || "Valued Customer"}</p>
          ...
          {/* 3. Itemized Taxable Goods/Services Table */}
          <div className="invoice-block border border-slate-300 rounded-xl overflow-x-auto shadow-sm">
            <table className="w-full min-w-[440px] sm:min-w-0 text-left border-collapse text-[10.5px]">
              <thead>
                <tr className="bg-slate-900 text-white text-[10px] font-bold uppercase tracking-wider">
                  <th className="p-2 text-center w-10">#</th>
                  <th className="p-2">Description of Service</th>
                  <th className="p-2 text-center">SAC Code</th>
                  <th className="p-2 text-center">Qty / Days</th>
              ...
              <tbody className="divide-y divide-slate-200 text-slate-800">
                ...
                <tr>
                  ...
                  <td className="p-2 text-center font-mono font-bold text-slate-700">996601</td>
                  <td className="p-2 text-center font-bold">{rentalDays} Days</td>
                  <td className="p-2 text-right font-bold text-slate-950">₹{baseRental.toLocaleString("en-IN")}</td>
                </tr>
                ...
                {deliveryFee > 0 && (
                  <tr>
                    ...
                    <td className="p-2">
                      ...
                      <p className="text-[9.5px] text-slate-500">Tirupati Airport / Hotel doorstep staging</p>
                    </td>
                    <td className="p-2 text-center font-mono font-bold text-slate-700">996601</td>
                    <td className="p-2 text-center font-bold">1 Trip</td>
                    <td className="p-2 text-right font-bold text-slate-950">₹{deliveryFee.toLocaleString("en-IN")}</td>
                  </tr>
                )}
                ...
                {discountAmount > 0 && (
                  <tr className="bg-emerald-50/60 text-emerald-900">
                    <td className="p-2 text-center font-bold text-emerald-700">•</td>
                    <td className="p-2">
                      <p className="font-bold">Promotional Discount / Coupon Concession</p>
                      <p className="text-[9.5px] text-emerald-700">Special seasonal trip discount applied</p>
                    </td>
                    <td className="p-2 text-center font-mono">996601</td>
          ...
          <div className="invoice-block grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
            ...
            <div className="md:col-span-7 space-y-2.5">
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 space-y-0.5">
                ...
                <p className="font-black text-slate-950 italic text-[11px]">{amountInWords}</p>
              </div>

              {/* Settlement / Razorpay Ledger */}
              <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase text-amber-950">Payment & Escrow Summary</span>
                  <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-800 font-bold">
                    ● Razorpay Verified
                  </span>
                </div>

                <div className="grid grid-cols-2 gap-2 text-[10px] pt-1 border-t border-amber-500/20">
                  <div>
                    <span className="text-slate-600 block text-[9.5px]">Advance Paid Online:</span>
                    <strong className="text-emerald-700 font-black text-xs">
                      ₹{paidAmount.toLocaleString("en-IN")}
                    </strong>
                    <p className="text-[8.5px] font-mono text-slate-500 truncate">Txn: {transactionId}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-slate-600 block text-[9.5px]">Balance Due at Handover:</span>
                    <strong className="text-amber-800 font-black text-xs">
                      ₹{balanceDue.toLocaleString("en-IN")}
                    </strong>
                    <p className="text-[8.5px] text-slate-500">Payable via UPI/Card at station</p>
                  </div>
                </div>

                <div className="text-[9.5px] text-slate-600 pt-1 flex justify-between border-t border-amber-500/20">
            ...
            <div className="md:col-span-5 p-3 rounded-xl bg-slate-900 text-white space-y-1.5">
              ...
              <div className="pt-1.5 border-t border-slate-700 flex justify-between items-center text-xs font-black">
                <span className="text-white uppercase tracking-wide">Total Invoice Value:</span>
                <span className="text-amber-400 text-sm font-mono">₹{grandTotal.toLocaleString("en-IN")}</span>
              </div>
            </div>
          </div>

          {/* 5. Statutory Declaration, Signatory & QR Seal */}
          <div className="invoice-block pt-2.5 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-12 gap-3 items-center">
            <div className="sm:col-span-8 space-y-0.5 text-[9px] text-slate-500 leading-tight">
            ...
            <div className="sm:col-span-4 p-2 rounded-xl bg-slate-50 border border-slate-200 text-center space-y-0.5">
              <div className="inline-flex items-center gap-1 text-[9px] font-bold text-emerald-700">
                <ShieldCheck className="w-3 h-3" /> Digitally Certified
              </div>
              <p className="text-[9px] font-black text-slate-900 uppercase">
                MOAR CARS PRIVATE LIMITED
              </p>
              <p className="text-[8px] text-slate-500">Authorized Fleet Operations Desk</p>
        ...
        <div className="px-5 py-3 bg-slate-100 border-t border-slate-200 flex justify-end gap-2.5 print:hidden">
          <Button
            onClick={onClose}
            variant="outline"
            className="h-9 px-5 rounded-xl border-slate-300 text-slate-700 hover:bg-slate-200 font-bold text-xs"
          >
            Close Invoice
          </Button>
          <Button
            onClick={handlePrint}
            className="h-9 px-5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-black text-xs uppercase tracking-wide flex items-center gap-2 shadow"
          >
            <Printer className="h-3.5 w-3.5 text-amber-400" /> Print Official Tax Invoice
          </Button>
        </div>
