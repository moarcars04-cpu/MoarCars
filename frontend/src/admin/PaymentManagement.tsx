      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <h2 className="text-2xl font-black flex items-center gap-2 text-white">
            <CreditCard className="w-6 h-6 text-[#c88d18]" /> Payments Ledger & Security Escrow
          </h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-slate-900 border border-slate-800 text-[#c88d18] font-bold text-xs hover:bg-[#c88d18]/20"
          >
          ...
          {["all", "Captured", "Pending", "Refunded", "Partial"].map((st) => (
            <button
              ...
              className={`px-3 py-1 rounded-xl text-[11px] font-bold ${
                filterStatus === st
                  ? "bg-slate-900 text-[#c88d18] border border-amber-400/40"
          ...
          <tbody className="divide-y divide-slate-800">
            {filteredPayments.map((p) => (
              <tr key={p.id} className="hover:bg-slate-800/60 transition-colors">
                <td className="px-5 py-4 font-mono font-bold text-[#c88d18]">{p.id}</td>
                ...
                <td className="px-5 py-4">
                  ...
                  <p className="text-[9px] text-slate-400 font-mono truncate max-w-[120px] mt-0.5">{p.transactionId}</p>
                </td>
                <td className="px-5 py-4">
                  <span
                    className={`px-2.5 py-1 rounded-xl text-[10px] font-bold border ${
                      p.status === "Captured"
                        ...
                        : p.status === "Refunded"
                        ? "bg-[#c88d18]/15 text-[#c88d18] border-amber-500/30"
                ...
                <td className="px-5 py-4 text-right">
                  {p.status === "Captured" && (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-[#c88d18] to-[#d49b29] text-slate-950 font-black text-[10px] shadow-md hover:opacity-95"
                    >
                      Release Deposit
                    </button>
                  )}
          ...
          <div className="bg-[#0b1426] border border-slate-800 rounded-3xl w-full max-w-md p-6 shadow-2xl text-white space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-slate-800">
              <h3 className="text-base font-black flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-[#c88d18]" /> Release Security Deposit: Booking #{selectedPaymentForRefund.bookingId}
              </h3>
              <button onClick={() => setSelectedPaymentForRefund(null)} className="text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="p-4 rounded-2xl bg-[#070e1c] border border-slate-800 space-y-1 text-xs">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-400 font-bold mb-1">Deduct Penalty / Damage Surcharge (₹)</label>
                <input
                  type="number"
                  value={damageDeduction}
                  onChange={(e) => setDamageDeduction(parseInt(e.target.value) || 0)}
                  placeholder="0"
                  className="w-full bg-[#070e1c] border border-slate-800 rounded-xl p-2.5 text-red-400 font-bold"
                />
              </div>

              {damageDeduction > 0 && (
                <div>
                  <label className="block text-slate-400 font-bold mb-1">Deduction Reason</label>
                  <input
                    type="text"
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-800">
              ...
              <button
                ...
                className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#c88d18] to-[#d49b29] text-slate-950 font-black text-xs shadow-lg shadow-amber-500/20"
              >
